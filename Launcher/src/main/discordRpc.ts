    ...
    try {
      ...
      try {
        ...
        if (!loggedSetActivityHint) {
          ...
          logMain(
            'info',
            'Vérifiez : Discord ouvert, partage d’activité activé (Paramètres Discord → Confidentialité), images Rich Presence sur le portail développeur (clés logo, solea_pack_palamod, solea_pack_wither, solea_pack_mythic_trials, solea_pack_mythic_trials_2).'
          )
        }
      }
    }
