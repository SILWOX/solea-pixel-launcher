...
import { LAUNCHER_VERSION_LABEL } from './launcherVersionLabel'
...
function IconCreditsBuild({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" width={20} height={20} fill="none" aria-hidden>
      <path
        d="M8.5 4.5h7l3 3v12a1 1 0 01-1 1h-11a1 1 0 01-1-1v-14a1 1 0 011-1z"
        stroke="currentColor"
        strokeWidth="1.6"
        strokeLinejoin="round"
      />
...
export function CreditsModal({ open, onClose }: CreditsModalProps) {
  ...
  useEffect(() => {
    if (!open) return
    let cancelled = false
    void window.solea.getAppVersion().then((v) => {
      if (!cancelled) setPackageVersion(v.trim())
    })
    return () => {
      cancelled = true
    }
  ...
  const buildDisplay =
    packageVersion && packageVersion.length > 0 ? `${LAUNCHER_VERSION_LABEL} · ${packageVersion}` : LAUNCHER_VERSION_LABEL
  ...
  return (
    <div className="pack-confirm-backdrop credits-backdrop" role="presentation" onClick={onClose}>
      <div
        ref={dialogRef}
        className="pack-confirm-modal credits-modal solea-modal-surface"
        role="dialog"
        aria-modal="true"
            ...
            <div className="credits-cards">
              ...
              <article className="credits-card credits-card--ui">
                ...
                <div className="credits-card-main">
                  ...
                  <div className="credits-card-stack">
                    <p className="credits-card-text">{t('settings.creditsUiIntro')}</p>
                    <p className="credits-card-text">{t('settings.creditsUiSub')}</p>
                  </div>
                </div>
              </article>
              ...
              <article className="credits-card">
                ...
                <div className="credits-card-main">
                  <h3 className="credits-card-title">{t('settings.creditsTechTitle')}</h3>
                  <p className="credits-card-text">{t('settings.creditsTechBody')}</p>
                </div>
              </article>

              <article className="credits-card credits-card--build">
                <div className="credits-card-icon" aria-hidden>
                  <IconCreditsBuild />
                </div>
                <div className="credits-card-main">
                  <h3 className="credits-card-title">{t('settings.creditsBuildTitle')}</h3>
                  <p className="credits-card-text credits-card-text--mono">{buildDisplay}</p>
                  <p className="credits-card-text credits-card-hint">{t('settings.creditsBuildHint')}</p>
                </div>
              </article>
            </div>
